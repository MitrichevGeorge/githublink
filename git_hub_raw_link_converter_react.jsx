...
// Exports a modern page that converts GitHub blob/raw file URLs into raw.githubusercontent.com links

export default function GitHubRawConverter() {
  ...
  useEffect(() => {
    const handleMove = (e) => {
      // put cursor position into css variables for the glow effect
      const x = e.clientX;
      const y = e.clientY;
  ...
  function convertGitHubUrl(url) {
    ...
    try {
      ...
      // If it's already raw.githubusercontent — return as-is
      if (u.hostname === "raw.githubusercontent.com") return url.trim();

      // Patterns to handle:
      // https://github.com/:owner/:repo/blob/:branch/path/to/file
      // https://github.com/:owner/:repo/raw/:branch/path/to/file
      // https://github.com/:owner/:repo/:branch/path/to/file (less common)

      const parts = u.pathname.split("/").filter(Boolean); // remove empty
      // expected: [owner, repo, blob|raw|...]
      if (parts.length < 3) {
        setError("Couldn't parse GitHub path.");
        return "";
      }
      ...
      // find index of 'blob' or 'raw'
      let markerIndex = parts.indexOf("blob");
      if (markerIndex === -1) markerIndex = parts.indexOf("raw");

      // If marker not present assume third segment is branch name (dangerous but try)
      let branch = "";
      let filepath = "";
      ...
      if (markerIndex !== -1) {
        ...
        // treat parts[2] as branch and rest as path
        branch = parts[2] || "main";
        filepath = parts.slice(3).join("/");
      }

      if (!filepath) {
        setError("No file path detected in the URL.");
        return "";
      }
  ...
  function handleConvert(e) {
    e && e.preventDefault();
    const r = convertGitHubUrl(input);
    setOutput(r);
    // focus output for easy copy
    setTimeout(() => outRef.current && outRef.current.select(), 50);
  }
  ...
  function handleCopy() {
    ...
    navigator.clipboard.writeText(output).then(() => {
      ...
      const prev = outRef.current?.value;
      outRef.current && (outRef.current.value = "Copied! ✅");
      setTimeout(() => (outRef.current && (outRef.current.value = prev)), 900);
    });
  }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-black p-6 relative overflow-hidden text-white">
      ...
      {/* Main glass panel - fixed the broken JSX by keeping only one properly quoted className */}
        ...
        <form onSubmit={handleConvert} className="space-y-4">
          ...
          <div className="flex gap-3">
            <input
              ...
              className="flex-1 rounded-lg px-4 py-3 bg-white/6 placeholder-white/40 outline-none border border-white/6 focus:border-indigo-400"
              placeholder="https://github.com/owner/repo/blob/main/path/to/file.sh"
            />
          ...
          <div className="mt-4 text-sm text-white/60">
            Examples supported: <code>/blob/</code>, <code>/raw/</code> and many common GitHub file links. If branch can't be detected, <strong>main</strong> is assumed.
          </div>

          <hr className="my-4 border-white/6" />

          <div className="flex items-center justify-between text-sm text-white/70">
            <div className="text-xs text-white/50">Tip: hit Enter to convert</div>
          </div>
        </form>
      </main>
